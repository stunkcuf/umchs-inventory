...
if fly volumes list --app umchs-inventory-goypog 2>/dev/null | grep -q "umchs_inventory_data"; then
...
if fly secrets list --app umchs-inventory-goypog 2>/dev/null | grep -q "JWT_SECRET"; then
    echo "✅ JWT_SECRET already set"
else
    echo "⚠️  JWT_SECRET not set. Generating secure secret..."
    JWT_SECRET=$(openssl rand -base64 32)
    fly secrets set JWT_SECRET="$JWT_SECRET" --app umchs-inventory-goypog
